...
def submit_file_path_to_workflow(file_path, args = { })
  ...
  external_parameters[file_path_parameter_name] = file_path if file_path_parameter_name
...
op.on('--file-path-parameter-name NAME', 'The name of the parameter to submit the file path to.') { |v| options[:file_path_parameter_name] = v }
...
options[:external_arguments] = additional_arguments

abort("workflow-id is a required argument.\n#{op.to_s}") unless options[:workflow_id]
#abort("file-path-parameter-name is a required argument.\n#{op.to_s}") unless options[:file_path_parameter_name]

initialize_orchestrator(args.merge(options))

file_paths = remaining_arguments
[*file_paths].each do |file_path|
  _file_paths = Dir.glob(file_path)
  _file_paths.each { |_file_path| submit_file_path_to_workflow(_file_path, options) }
end
